...
export async function POST(req: NextRequest) {
  ...
  const event = stripe.webhooks.constructEvent(body, signature, WEBHOOK_SECRET_KEY)
  switch (event.type) {
    // this is the thing that go through when the payment is successful
...
const handleChargeSucceeded = async (charge: Stripe.Charge) => {
  ...
  await engine.erc1155.mintTo('mumbai', NEXT_PUBLIC_NFT_CONTRACT_ADDRESS, BACKEND_WALLET_ADDRESS, {
